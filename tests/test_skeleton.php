...
$root = dirname(__DIR__) . '/plugins/magnolia_mcp';
$yaml = file_get_contents($root . '/magnolia_mcp.yaml');
mcp_test_expect($yaml !== false, 'yaml exists');
mcp_test_expect((bool) preg_match('/^name: magnolia_mcp\s*$/m', $yaml), 'yaml name unquoted magnolia_mcp');
...
mcp_test_expect((bool) preg_match('/^info_url: \/plugins\/magnolia_mcp\/pages\/help\.php\s*$/m', $yaml), 'yaml info_url is help page');
mcp_test_expect(!str_contains($yaml, "name: '"), 'yaml values are unquoted');

require $root . '/config/config.php';
mcp_test_expect_eq($magnolia_mcp_enable ?? null, true, 'enable default true');
mcp_test_expect_eq($magnolia_mcp_trust_proxy ?? null, false, 'trust proxy default off');
mcp_test_expect_eq($magnolia_mcp_allow_uncurated ?? null, false, 'uncurated default off');

require $root . '/languages/en.php';
mcp_test_expect_eq($lang['magnolia_mcp_title'] ?? null, 'MCP Server', 'lang title is MCP Server');

$hook = $root . '/hooks/all.php';
mcp_test_expect(file_exists($hook), 'hooks/all.php exists');
require_once $hook;
mcp_test_expect(function_exists('HookMagnolia_mcpAllExtra_checks'), 'extra_checks hook defined');
mcp_test_expect(function_exists('HookMagnolia_mcpAllUser_home_additional_links'), 'user home help link hook defined');
mcp_test_expect(function_exists('HookMagnolia_mcpAllCustomteamfunction'), 'team centre tile hook defined');
mcp_test_expect(function_exists('HookMagnolia_mcpAllPreheaderoutput'), 'totp return hook defined');
mcp_test_expect(isset($lang['magnolia_mcp_help_title']), 'help title lang set');
mcp_test_expect(isset($lang['magnolia_mcp_help_step2']), 'help step2 lang set');
...
foreach (['do_search', 'create_resource', 'upload_file_by_url', 'update_field', 'create_collection', 'checkperm', 'get_system_status'] as $id) {
    mcp_test_expect(isset($magnolia_mcp_annotations[$id]), 'annotation ' . $id);
}
mcp_test_expect_eq($magnolia_mcp_annotations['do_search']['category'], 'search', 'do_search category');
...
mcp_test_expect(str_contains($setup_src, "'magnolia_mcp_allow_' . \$cat"), 'setup allowlist per category');
...
mcp_test_expect(str_contains($setup_src, 'magnolia_mcp_oauth_nginx'), 'setup shows nginx snippet');
...
mcp_test_expect(str_contains($setup_src, 'magnolia_mcp_oauth_revoke_confirm'), 'setup confirms before revoke');
mcp_test_expect(str_contains($setup_src, 'pages/help.php'), 'setup links to help page');
mcp_test_expect(str_contains($setup_src, 'magnolia_mcp_trust_proxy_needed'), 'setup warns when proxy HTTPS is untrusted');
...
mcp_test_expect(str_contains($help_src, 'magnolia_mcp_help_prereq'), 'help mentions admin prerequisites');
mcp_test_expect(str_contains($lang['magnolia_mcp_help_prereq'] ?? '', '/mcp'), 'help prereq names the /mcp rewrite');
mcp_test_expect(str_contains($lang['magnolia_mcp_oauth_discovery_help'] ?? '', '/mcp'), 'discovery help names the /mcp rewrite');
...
mcp_test_expect(str_contains($help_src, 'magnolia_mcp_help_bearer'), 'help documents API-key Bearer');
...
mcp_test_expect(!str_contains($lang['magnolia_mcp_oauth_discovery_help'] ?? '', 'Not required'), 'discovery help does not call origin well-known optional');
...
foreach ($expected_ann as $category => $ids) {
    foreach ($ids as $id) {
        mcp_test_expect(isset($magnolia_mcp_annotations[$id]), 'annotation ' . $id);
        mcp_test_expect_eq($magnolia_mcp_annotations[$id]['category'] ?? null, $category, $id . ' category');
        $row = $magnolia_mcp_annotations[$id];
...
mcp_test_expect(!isset($magnolia_mcp_annotations['login']), 'login not annotated');
mcp_test_expect(!isset($magnolia_mcp_annotations['validate_upload_url']), 'validate_upload_url not annotated');
mcp_test_expect(!isset($magnolia_mcp_annotations['do_report']), 'do_report not annotated');
mcp_test_expect_eq($magnolia_mcp_annotations['get_resource_access']['category'], 'resources', 'get_resource_access in resources');
mcp_test_expect_eq($magnolia_mcp_annotations['get_edit_access']['category'], 'resources', 'get_edit_access in resources');
mcp_test_expect_eq($magnolia_mcp_annotations['delete_resource']['destructiveHint'], true, 'delete_resource destructive');
mcp_test_expect_eq($magnolia_mcp_annotations['create_resource']['destructiveHint'], false, 'create_resource not destructive');
mcp_test_expect_eq($magnolia_mcp_annotations['new_user']['destructiveHint'], true, 'new_user destructive');
mcp_test_expect_eq($magnolia_mcp_annotations['save_user']['destructiveHint'], true, 'save_user destructive');

$prev_https = $_SERVER['HTTPS'] ?? null;
$prev_remote = $GLOBALS['enable_remote_apis'] ?? null;
$prev_enable = $GLOBALS['magnolia_mcp_enable'] ?? null;
$prev_base = $GLOBALS['baseurl'] ?? null;
$prev_trust = $GLOBALS['magnolia_mcp_trust_proxy'] ?? null;
$GLOBALS['enable_remote_apis'] = false;
$GLOBALS['magnolia_mcp_enable'] = true;
$GLOBALS['baseurl'] = 'https://dam.example';
$GLOBALS['magnolia_mcp_trust_proxy'] = false;
unset($_SERVER['HTTPS']);
$fail_checks = HookMagnolia_mcpAllExtra_checks();
mcp_test_expect_eq($fail_checks['magnolia_mcp']['status'] ?? null, 'FAIL', 'extra_checks fails when remote apis off');
...
$ok_checks = HookMagnolia_mcpAllExtra_checks();
mcp_test_expect_eq($ok_checks['magnolia_mcp']['status'] ?? null, 'OK', 'extra_checks ok on CLI when baseurl is https');
$GLOBALS['baseurl'] = 'http://dam.example';
$_SERVER['HTTPS'] = 'on';
$http_checks = HookMagnolia_mcpAllExtra_checks();
mcp_test_expect_eq($http_checks['magnolia_mcp']['status'] ?? null, 'FAIL', 'extra_checks fails when baseurl is http');
...
$GLOBALS['magnolia_mcp_enable'] = $prev_enable;
$GLOBALS['baseurl'] = $prev_base;
$GLOBALS['magnolia_mcp_trust_proxy'] = $prev_trust;

$prev_csrf = $GLOBALS['CSRF_exempt_pages'] ?? null;
$prev_csrf_remote = $GLOBALS['enable_remote_apis'] ?? null;
$prev_csrf_enable = $GLOBALS['magnolia_mcp_enable'] ?? null;
$prev_csrf_base = $GLOBALS['baseurl'] ?? null;
$GLOBALS['enable_remote_apis'] = true;
$GLOBALS['magnolia_mcp_enable'] = true;
$GLOBALS['baseurl'] = 'https://dam.example';
$GLOBALS['CSRF_exempt_pages'] = ['login', 'mcp'];
$csrf_fail = HookMagnolia_mcpAllExtra_checks();
mcp_test_expect_eq($csrf_fail['magnolia_mcp']['status'] ?? null, 'FAIL', 'extra_checks fails when mcp_upload not CSRF-exempt');
mcp_test_expect(str_contains((string) ($csrf_fail['magnolia_mcp']['info'] ?? ''), 'mcp_upload'), 'extra_checks names mcp_upload');
$GLOBALS['CSRF_exempt_pages'] = ['login', 'mcp', 'mcp_upload'];
$csrf_ok = HookMagnolia_mcpAllExtra_checks();
mcp_test_expect_eq($csrf_ok['magnolia_mcp']['status'] ?? null, 'OK', 'extra_checks ok when mcp_upload exempt');
$GLOBALS['CSRF_exempt_pages'] = ['mcp', 'mcp_upload'];
$login_fail = HookMagnolia_mcpAllExtra_checks();
mcp_test_expect_eq($login_fail['magnolia_mcp']['status'] ?? null, 'FAIL', 'extra_checks fails when login dropped from CSRF exempt');
$_SERVER['HTTP_X_FORWARDED_PROTO'] = 'https';
unset($_SERVER['HTTPS']);
$GLOBALS['CSRF_exempt_pages'] = ['login', 'mcp', 'mcp_upload'];
$GLOBALS['magnolia_mcp_trust_proxy'] = false;
$GLOBALS['baseurl'] = 'https://dam.example';
$proxy_fail = HookMagnolia_mcpAllExtra_checks();
mcp_test_expect_eq($proxy_fail['magnolia_mcp']['status'] ?? null, 'FAIL', 'extra_checks fails when proxy HTTPS is untrusted');
...
mcp_test_expect(!isset($lang['magnolia_mcp_php_upload_max']), 'php upload_max lang not set');
mcp_test_expect(!isset($lang['magnolia_mcp_help_upload_title']), 'help upload title lang not set');
mcp_test_expect(!isset($lang['magnolia_mcp_upload_hosts']), 'upload hosts lang not set');
if ($prev_csrf === null) {
...
$GLOBALS['magnolia_mcp_enable'] = $prev_csrf_enable;
